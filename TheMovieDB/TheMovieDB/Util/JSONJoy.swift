...
open class JSONDecoder {
	...
	public init(_ raw: AnyObject) {
		...
		if let data = rawObject as? Data {
			...
			do {
				try response = JSONSerialization.jsonObject(with: data, options: JSONSerialization.ReadingOptions()) as! [String:AnyObject] as AnyObject
				rawObject = response!
			}
	...
	open func print() -> String {
		...
		if value != nil {
			...
			else if value?.absoluteString == nil{
				return "null"
			}
			return "\(value!)"
		}
